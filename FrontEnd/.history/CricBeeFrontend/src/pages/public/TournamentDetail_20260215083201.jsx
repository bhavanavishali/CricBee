        ...
        <div className="max-w-7xl mx-auto px-4 py-8">
          ...
          <div className="bg-white rounded-lg shadow-sm mb-6">
            <div className="border-b border-gray-200">
              <div className="flex space-x-8 px-6">
                {['overview', 'fixtures', 'players', 'live', 'fans'].map((tab) => (
                  <button
                    key={tab}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-2 space-y-6">
              {/* Fixtures Tab - Show Matches */}
              {activeTab === 'fixtures' && tournament.matches && tournament.matches.length > 0 && (
                <div className="bg-white rounded-lg shadow-sm p-6">
                  <h2 className="text-2xl font-bold text-gray-900 mb-4">Tournament Fixtures</h2>
                  <div className="space-y-4">
                    {tournament.matches.map((match) => (
                      <div key={match.id} className="border border-gray-200 rounded-lg p-4 hover:border-teal-500 transition">
                        <div className="flex items-center justify-between">
                          <div className="flex-1">
                            <div className="flex items-center gap-2 mb-2">
                              <span className={`px-2 py-1 rounded text-xs font-semibold ${
                                match.status_badge === 'live' 
                                  ? 'bg-red-100 text-red-700' 
                                  : match.status_badge === 'completed'
                                  ? 'bg-gray-100 text-gray-700'
                                  : 'bg-blue-100 text-blue-700'
                              }`}>
                                {match.status_badge === 'live' ? '● LIVE' : match.status_badge?.toUpperCase() || 'SCHEDULED'}
                              </span>
                              {match.round_name && (
                            ...
                            <div className="text-sm text-gray-600">
                              {match.venue} • {match.match_date} at {match.match_time}
                            </div>
                          </div>
                          <div className="flex gap-2">
                            {(match.status_badge === 'live' || match.status_badge === 'ongoing') && match.streaming_url && (
                              <button
