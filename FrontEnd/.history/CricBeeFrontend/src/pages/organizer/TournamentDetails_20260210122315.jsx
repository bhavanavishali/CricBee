...
const TournamentDetails = () => {
  ...
  const getStatusBadge = (status) => {
    const statusMap = {
      ...
      'tournament_end': { label: 'Tournament Completed', color: 'bg-green-100 text-green-800' },
  ...
  // Calculate Points Table (mock implementation - in production would come from backend)
                    ...
                    <div className="flex items-center gap-4">
                      <div className="w-16 h-16 bg-indigo-100 rounded-full flex items-center justify-center">
                        <span className="text-2xl font-bold text-indigo-600">
                          {tournament.organizer?.organization?.organization_name?.charAt(0) || 'O'}
                        </span>
                      </div>
                      <div>
                        <p className="font-semibold text-gray-900 flex items-center gap-2">
                          {tournament.organizer?.organization?.organization_name || 'Tournament Organizer'}
                          {tournament.organizer?.is_verified && (
                        ...
                        {tournament.details?.location && (
                          <p className="text-sm text-gray-500 mt-1">Location: {tournament.details.location}</p>
                        )}
                      </div>
                    </div>
                  </div>
