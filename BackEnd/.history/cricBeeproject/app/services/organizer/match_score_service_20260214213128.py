...
def update_score(
    ...
    # Get the last ball bowled in the CURRENT INNINGS only
    # First, determine if we're in first or second innings
    ...
    # Get max overs from tournament to distinguish innings
    max_overs = tournament_details.overs if tournament_details else 20
    
    if is_second_innings:
        # We're in second innings - only look at balls from second innings
        # Second innings balls have over_number > max_overs (first innings max overs)
        ...
        if not last_ball:
            # No balls in second innings yet - start from over 1
            current_over = 1
            current_ball = 1
            actual_over_number = max_overs + 1
        else:
            display_over = last_ball.over_number - max_overs  # Convert to display over (1-based)
            
            # Always count legal balls to determine current ball number
            ...
            # Fix: For second innings, actual_over_number should be max_overs + display_over
            # This ensures over numbers restart from 1 in display but are stored uniquely
            actual_over_number = max_overs + display_over
    else:
        # We're in first innings - only look at balls from first innings
        last_ball = db.query(BallByBall).filter(
            BallByBall.match_id == match_id,
            BallByBall.over_number <= max_overs
        ).order_by(
