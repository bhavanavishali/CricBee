...
def check_round_completion(db: Session, round_id: int, tournament_id: int = None) -> Dict:
    ...
    Returns dict with completion status and match statistics.
    """
    print(f"DEBUG: Checking round completion for round_id: {round_id}, tournament_id: {tournament_id}")
    
    # Filter by round_id and tournament_id if provided
    ...
    print(f"DEBUG: Found {len(matches)} matches for round_id: {round_id}, tournament_id: {tournament_id}")
    ...
    print(f"DEBUG: Round {round_id} - Total: {total_matches}, Completed: {completed_matches}")
