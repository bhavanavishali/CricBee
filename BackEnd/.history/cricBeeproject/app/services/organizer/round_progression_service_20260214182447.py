...
def save_qualified_teams(
    ...
    # Use the tournament's actual organizer_id instead of the passed one
...
def get_qualified_teams(
    ...
    """
    Get list of qualified team IDs for a specific round progression.
    """
...
def get_all_qualified_teams_for_round(
    ...
    """
    Get all qualified teams for a specific target round (regardless of source round).
    """
    progressions = db.query(RoundProgression).filter(
        RoundProgression.tournament_id == tournament_id,
        RoundProgression.to_round == to_round
...
def complete_tournament_with_winner(
    ...
    """
    Mark tournament as completed with a winner (for Round 3 completion).
    """
    ...
    print(f"DEBUG: Before update - winner_team_id: {tournament.winner_team_id}, status: {tournament.status}")
    print(f"DEBUG: TournamentStatus.COMPLETED.value = '{TournamentStatus.COMPLETED.value}'")
    tournament.winner_team_id = winner_team_id
    tournament.status = TournamentStatus.COMPLETED.value
    print(f"DEBUG: After update - winner_team_id: {tournament.winner_team_id}, status: {tournament.status}")
    
    db.commit()
    db.refresh(tournament)
    print(f"DEBUG: After commit - winner_team_id: {tournament.winner_team_id}, status: {tournament.status}")
    ...
    print(f"DEBUG: Direct DB query - status: '{db_status[0]}', winner_team_id: {db_status[1]}")
