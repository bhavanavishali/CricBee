...
def initialize_default_rounds(
    ...
    # Get the 3 global rounds (they should always exist)
...
def update_round_name(
    ...
    # Verify organizer has access (check if they have any tournaments)
    # Since rounds are global, we just verify the organizer exists
...
def create_fixture_round(
    ...
    # Since rounds are now global, we cannot create new rounds
    # Only the 3 default rounds (Round 1, 2, 3) exist
...
def get_tournament_rounds(
    ...
    # Return the 3 global rounds (all tournaments share the same rounds)
    rounds = db.query(FixtureRound).filter(
        FixtureRound.round_no.in_([1, 2, 3])
    ).order_by(FixtureRound.round_no.asc()).all()
    
    # Ensure rounds exist, if not create them
    if len(rounds) < 3:
...
def create_match(
    ...
    # Verify round exists (rounds are now global)
    ...
    # Verify both teams are enrolled in the tournament with successful payment
    ...
    try:
        ...
        # Automatically initialize points table when first match is created
        try:
            # Check if points table already exists for this tournament
            ...
            if not existing_points:
                # Initialize points table with all enrolled clubs
...
def get_round_matches(
    ...
    # Get the round (rounds are now global)
    round = db.query(FixtureRound).filter(FixtureRound.id == round_id).first()
    
    if not round:
        raise ValueError("Round not found")
    
    # If tournament_id is provided, verify organizer owns it
    if tournament_id:
        tournament = db.query(Tournament).filter(
            Tournament.id == tournament_id,
            Tournament.organizer_id == organizer_id
        ...
        # Get matches for this round and tournament
        ...
        # Get all matches for this round (across all tournaments)
        # Verify organizer has at least one tournament
...
def update_match_details(
    ...
    # Update date, time, venue, and streaming_url
...
def initialize_league_fixture_structure(
    ...
    # Initialize league fixture structure - rounds are now global, just return them
    ...
    # Get or create the 3 global rounds
    rounds = initialize_default_rounds(db, tournament_id, organizer_id)
    
    # Update round names for League mode
...
def generate_league_matches(
    ...
    # Get round (rounds are now global)
    round = db.query(FixtureRound).filter(FixtureRound.id == round_id).first()
    
    if not round:
        raise ValueError("Round not found")
    
    # Verify it's Round 1 (League round)
    if round.round_no != 1:
    ...
    # Get all enrolled teams with successful payment first
    ...
    # Calculate expected number of matches using Single Round-Robin formula: n*(n-1)/2
    expected_match_count = len(team_ids) * (len(team_ids) - 1) // 2
    
    # Check if matches already exist for this round and tournament
    ...
    # If we have the expected number of matches or more, return them
