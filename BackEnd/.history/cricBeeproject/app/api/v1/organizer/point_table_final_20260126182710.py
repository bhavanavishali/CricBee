...
def get_point_table(tournament_id: int, db: Session = Depends(get_db)):
    """
    Get point table for a specific tournament.
    Returns sorted list by points, NRR, and wins.
    """
    try:
        # Debug: First check all enrollments for this tournament
        ...
        # Get all enrolled clubs for the tournament (more flexible - include any payment status)
        ...
        # Check if point table entries exist
        point_entries = db.query(PointTable).filter(
            PointTable.tournament_id == tournament_id
        ).all()
        
        # Create point table entries if they don't exist
        ...
        if point_entries:
            ...
            # Just show enrolled clubs with 0 values
...
def initialize_point_table(tournament_id: int, db: Session = Depends(get_db)):
    """
    Initialize point table for a tournament with all enrolled clubs.
    Only organizer of the tournament can initialize.
    """
    try:
        from app.services.organizer import point_table_service
