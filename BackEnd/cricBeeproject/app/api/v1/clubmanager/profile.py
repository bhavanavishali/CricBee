# api/v1/clubmanager/profile.py
"""
Club Profile Management Routes
Handles club profile CRUD operations and dashboard statistics
"""
...
def get_club_profile_endpoint(request: Request, db: Session = Depends(get_db)):
    """Get club profile for the current club manager"""
...
def get_dashboard_stats_endpoint(
    ...
    """Get dashboard statistics for the club manager"""
    current_user = get_current_user(request, db)
    if current_user.role != UserRole.CLUB_MANAGER:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
...
def update_profile_endpoint(
    ...
    """Update club profile (user and/or club details)"""
    current_user = get_current_user(request, db)
    if current_user.role != UserRole.CLUB_MANAGER:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
