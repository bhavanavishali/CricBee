...
#     """
#     user, otp = await register_user_with_otp(db, payload)
...
#             detail="User created but failed to send OTP email. Please request a new OTP."
#         )
...
#     """
#     Verify OTP sent to user's email during signup.
#     """
...
#     # Verify OTP
#     if not verify_user_otp(db, payload.email, payload.otp):
#         raise HTTPException(
#             status_code=status.HTTP_400_BAD_REQUEST,
...
def verify_otp(payload: OTPVerify, db: Session = Depends(get_db)):
    ...
    This is when the user is actually registered.
    """
